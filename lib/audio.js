...
class AudioManager {
  ...
  createChord(frequencies, duration) {
    return Promise.all(frequencies.map((freq) => this.createTone(freq, duration, "sine")))
  }
  ...
  async play(soundName, options = {}) {
    ...
    try {
      ...
      if (Array.isArray(sound)) {
        // 和弦或旋律
        if (sound.every((item) => typeof item === "object" && "frequency" in item)) {
          // 旋律
          sound.forEach((note) => {
            setTimeout(() => {
